...
## Passo 4, Receber a decisão do comitê

Se a sua política usa o [Comitê de Crédito IA](/concepts/comite-de-credito), registre também o tipo `COMMITTEE_FINISHED`. Cada webhook escuta **um** tipo, então é um registro separado:
...
curl -X POST https://gyra-core.gyramais.com.br/webhook \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "type": "COMMITTEE_FINISHED",
    "url": "https://seu-sistema.com/webhooks/gyra-comite"
  }'
```

O payload traz a decisão e a síntese do Presidente:
...
```json
{
  ...
  "data": {
    "content": {
      ...
      "status": "DONE",
      "decision": "APPROVED",
      "synthesis": { "resumo": "...", "conditions": ["..."], "confidence": 0.82 },
...
Se a política está em **Não convocar automaticamente**, o comitê só delibera depois que você chamar [`POST /report/{id}/committee`](/api-reference/report/post-reportcommittee). O gatilho natural é o próprio `REPORT_FINISHED`: quando ele chega, o relatório já está pronto para ir a comitê.

<Warning>
  Os webhooks da organização são copiados para o relatório **na criação dele**. Registre `COMMITTEE_FINISHED` antes de criar os relatórios que vão a comitê, senão o evento não tem destino para os que já existiam.
</Warning>
...
| Responder `200` antes de validar o conteúdo | Erro `4xx` (fora de `408`, `425` e `429`) não é retentado: o evento se perde no primeiro envio |
