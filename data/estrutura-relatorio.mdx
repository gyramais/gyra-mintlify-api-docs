...
| `yearMonthDay` | number | Data de referência como inteiro `AAAAMMDD` (ex.: `20260731`). |
| `organizationId` | string (ObjectId) | ID da organização que gerou o relatório |

| `status` | object | `{ id, value }`, com `value` em `PENDING`, `APPROVED` ou `DENIED`. |
| `policyId` / `policyVersion` / `policyName` | | Política aplicada. |
| `parecerEnabled` | boolean | Se o pipeline de parecer está habilitado. |
| `committeeStatus` / `committeeDecision` / `committeeResult` | | Estado e resultado do Comitê de Crédito IA. |
| `exportUrl` / `exportExcelUrl` | string | Últimas exportações geradas. |
...
| `selectedIntegrations` | string[] | Integrações que alimentaram esta seção |
| `sectionDetails` | object[] | **Os dados da seção.** Ver abaixo. |

### Onde ficam os dados: `sectionDetails[].values`

Cada item de `sectionDetails` tem `integrationTypes` (de onde veio) e `values` (o conteúdo).

<Warning>
  **O formato de `values` varia por seção.** Em algumas, os campos ficam no nível de cima. Em outras, ficam aninhados sob uma chave com o nome do provedor que respondeu (ex.: `values.SERASA`). Não existe uma regra única, então **não assuma nem uma coisa nem outra**: inspecione as chaves antes de acessar.
</Warning>

Exemplo real de uma seção com campos no nível de cima (`BAD_CHECK`):

```json
{
  "integrationCount": 1,
  "checksQuantity": 0,
  "checksBalance": "R$ 0,00",
  "lastCheckDate": "",
  "checkTable": []
}
```

Já os protestos via Serasa aninham sob a chave do provedor. O campo `integrationCount`, quando presente, indica quantas integrações contribuíram para aquele bloco.

O caminho robusto é inspecionar antes de acessar, em vez de assumir um caminho fixo:

```javascript
for (const detail of section.sectionDetails ?? []) {
  const v = detail.values ?? {};
  // campos no nível de cima?
  if ('checksQuantity' in v) usar(v);
  // ou aninhados sob o provedor?
  else for (const [provedor, dados] of Object.entries(v)) usar(dados, provedor);
}
```

<Tip>
  Se você quer a leitura consolidada e estável, independente de qual bureau respondeu, use [`GET /report/{id}/insights`](/api-reference/report/get-reportinsights) em vez de fazer parse de `values`.
</Tip>

### Campos crus e campos formatados

Vários tradutores devolvem o mesmo dado duas vezes: `value` e `date` (crus, para cálculo) mais `formattedValue` e `formattedDate` (formatados em português e padrão brasileiro, para exibição). Não faça parse do formatado para recuperar o número: use o cru.
