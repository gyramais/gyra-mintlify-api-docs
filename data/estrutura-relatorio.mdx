...
<Warning>
  **`values` é indexado pelo provedor que respondeu**, com a chave sendo o tipo de integração em camelCase. A mesma seção `PROTESTS` devolve `values.serasa` quando a política usa Serasa e outra chave quando usa outro provedor, cada uma com o seu conjunto de campos. **Não assuma uma chave fixa.**
</Warning>

```json
{
  "id": "69a207bcdd0197828df9156a",
  "type": { "id": "672b49a91acad6953b56b109", "value": "PROTESTS" },
  "selectedIntegrations": ["SERASA"],
  "sectionDetails": [
    {
      "integrationTypes": ["SERASA"],
      "values": {
        "serasa": {
          "protestsQuantity": 2,
          "protestsTotalValue": "R$ 12.400,00",
          "lastProtestDate": "2026-03-14",
          "protestsTable": []
        }
      }
    }
...
O caminho robusto é iterar, em vez de acessar uma chave fixa:

```javascript
for (const detail of section.sectionDetails ?? []) {
  for (const [provedor, dados] of Object.entries(detail.values ?? {})) {
    console.log(provedor, dados.protestsQuantity);
  }
}
```
