...
<Warning>
  **Página em revisão.** Os campos listados abaixo ainda não foram conferidos contra os tradutores de `CREDIT_POLICY` no backend, e sabemos que parte da documentação antiga descrevia campos que não correspondem à resposta real. Até a revisão terminar, confirme o formato consultando a seção diretamente com [`GET /report/{id}/section/{sectionType}`](/api-reference/report/get-report-section), ou use [`GET /report/{id}/insights`](/api-reference/report/get-reportinsights), que tem formato estável.
