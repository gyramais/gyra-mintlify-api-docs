...
- **Webhooks**, endpoints que recebem eventos em tempo real. Ver [Webhooks e API Keys](/toolbox/webhooks-e-api-keys).

<Note>
  **Não configura pelo toolbox:** API Keys (solicite ao suporte em `suporte@gyramais.com.br`), logo e branding da organização (configurado pela equipe da GYRA+ no provisionamento) e dados de faturamento (gestão via contrato, ainda sem tela no toolbox).
</Note>
...
| Papel | Vê |
| ----- | -- |
| **Administrador** | Tudo. Usuários, políticas, integrações, webhooks. |
| **Gestor de Crédito** | Políticas, templates, produtos, todas as análises. Não vê usuários. |
