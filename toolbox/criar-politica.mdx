...
<Steps>
  ...
  <Step title="Adicionar regras ao grupo">
    ...
    - **Regra**, escolhida no catálogo (ex: `SCORE`, `COMPANY_SITUATION`, `PEFIN_AMOUNT`). Lista completa em [Catálogo de Regras](/reference/catalogo-de-regras).
    - **Operador** e **valor** de comparação (ex: menor que `400`).
    - **Parâmetros**, quando a regra exigir (ex: o tipo de vara em `LAWSUITS_COUNT_BY_COURT_TYPE`).
    - **Status a aplicar**, `APPROVED`, `ALERT` ou `DENIED` quando a condição é satisfeita.
...
Todas as regras acima saem do [Catálogo de Regras](/reference/catalogo-de-regras).
