...
<Steps>
  <Step title="Começar pelos campos de situação">
    `COMPANY_SITUATION` e `DEATH` deveriam ser as primeiras regras de qualquer política: são bloqueios óbvios e baratos de avaliar.
  </Step>
  <Step title="Adicionar regras de idade">
    `COMPANY_OPENING_TIME` e `AGE_PERSON` são segundo filtro fundamental.
  </Step>
