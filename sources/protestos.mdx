...
| ----- | --------- | ------- |
| `protests.count` | Quantidade de protestos ativos | `3` |
| `protests.totalAmount` | Soma dos valores protestados | `45720.50` |
| `protests.items[].date` | Data do protesto | `"2025-11-12"` |
| `protests.items[].amount` | Valor do título | `15240.00` |
| `protests.items[].registryOffice` | Nome do cartório | `"1º Tabelião de Protesto de Belo Horizonte"` |
| `protests.items[].state` | UF | `"MG"` |
| `protests.items[].city` | Município | `"Belo Horizonte"` |
| `protests.items[].status` | Ativo ou quitado | `"ACTIVE"` / `"PAID"` |

Protestos **quitados** aparecem com `status: "PAID"` e ficam registrados por um período, úteis para histórico mesmo depois de resolvidos.
