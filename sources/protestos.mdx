...
- **Lista detalhada de ocorrências**: cada registro com cartório, comarca, UF, data de ocorrência, data de atualização, valor do título, credor/cedente, indicador de anuência, custas e status (ativo/baixado) conforme o bureau de origem.
...
<CardGroup cols={2}>
  <Card title="Bloqueio por protesto ativo" icon="ban">
    ...
    **Regra:** `NOTARIES_COUNT` maior que 0 : `DENIED`.
  </Card>
...
<Steps>
  <Step title="Regra binária simples">
    `NOTARIES_COUNT` maior que 0 : `DENIED`. Direto, conservador, funciona para crédito sensível.
  </Step>
  ...
  <Step title="Regra por quantidade + valor">
    Política mais refinada: `NOTARIES_COUNT` a partir de 3, ou `NOTARIES_AMOUNT` acima de 50.000 : `DENIED`.
  </Step>
