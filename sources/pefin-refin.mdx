...
<Steps>
  <Step title="Escolher o campo">
    Regras `PEFIN_AMOUNT` e `REFIN_AMOUNT` (valor), `PEFIN_LAST_DAYS` e `REFIN_LAST_DAYS` (recência) e `PEFIN_NOT_CALCULATED_REVENUE` / `REFIN_NOT_CALCULATED_REVENUE` (proporção do faturamento).
  </Step>
...
```json
{
  "rule": "PEFIN_NOT_CALCULATED_REVENUE",
