...
<Steps>
  ...
  <Step title="Rodar o comando manual">
    ...
    ```bash
    GYRA_API_KEY=gyr_live_... npx -y @gyramais/mcp-client --endpoint https://mcp.gyramais.com.br
    ```
...
- **API Key inválida ou com typo**. Copie novamente do toolbox.
- **API Key revogada**. Verifique em *Configurações, API Keys* se a chave ainda aparece como ativa.
- **API Key de organização errada**. Chave é vinculada a uma organização; se você tem várias, confira qual está usando.

### Solução

Gerar nova chave em *Configurações, API Keys*, atualizar no config do cliente e reiniciar.
...
Escopo da API Key não cobre o endpoint. Ex: chave somente leitura tentando `create_report`.

### Solução

Em *Configurações, API Keys*, edite os escopos da chave (ou crie uma nova com escopos completos).
...
Limite padrão é 100 req/min por API Key. Encadeamentos longos (ex: varrer 500 relatórios) excedem.
...
- **Fonte totalmente indisponível**: o status acaba em `ERROR`. Use `re_analyze_report` depois.
...
- API Key é por organização. Use uma config de MCP por organização, com nomes distintos (`gyra-orgA`, `gyra-orgB`).
