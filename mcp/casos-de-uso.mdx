...
>
> Quer que eu gere o PDF ou envie um link público para o cliente?
>
> **Analista**: Manda o link público.
>
> **Agente**: Link gerado, válido por 7 dias: https://toolbox.gyramais.com.br/public/report/...
...
5. `export_report_sync(reportId, "LINK")` — gerar link.
...
## Caso 3, reanálise em batch conversacional

### Contexto

Bureau caiu ontem à tarde e 15 análises ficaram com `ERROR`. Analista quer reanalisar tudo sem precisar clicar uma por uma.

### Conversa

> **Analista**: Quais análises ficaram com status ERROR ontem?
>
> **Agente**: Encontrei 15 análises com status `ERROR` entre 14:00 e 18:00 de ontem. Todas falharam na coleta do bureau.
>
> **Analista**: Reanalisa todas.
>
> **Agente**: Reanalisando... 15 de 15 concluídas. 14 ficaram `APPROVED`, 1 `DENIED` por protesto.

### Tools encadeadas

1. `list_reports({ status: "ERROR", from, to })`.
2. Loop com `re_analyze_report(reportId)` para cada.
3. `get_report(reportId)` em cada até completar.
...
>
> **Agente**: Encontrei 38 análises. Quer que eu exporte em CSV, gere um relatório consolidado ou rode insight agregado?
>
> **Auditor**: Insight agregado, foco em concentração por CNAE e score médio.
>
> **Agente**: Insight gerado. 3 CNAEs concentram 72% do volume aprovado: Comércio (34%), Serviços Empresariais (24%), Indústria Têxtil (14%). Score médio: 712, mediana: 698.

### Tools encadeadas

1. `list_reports({ status: "APPROVED", minAmount: 500000, policyId, from, to })`.
2. `generate_insight({ filters, type: "CONCENTRATION_CNAE" })`.
3. `review_insight(insightId)`.
