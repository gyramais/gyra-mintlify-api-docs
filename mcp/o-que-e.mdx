...
Toda tool exige autenticação com o **seu login da plataforma**: o mesmo e-mail e senha que você usa no toolbox. Duas formas de conectar:

1. **Custom connector (OAuth)** no Claude.ai/Claude Desktop: você adiciona o endpoint, clica em *Authenticate* e faz login na página da GYRA+. A senha não fica gravada em lugar nenhum do seu lado.
2. **Stdio local** em Claude Code, Cursor, VS Code, SDKs: e-mail e senha em variáveis de ambiente (`GYRA_CLIENT_ID` + `GYRA_CLIENT_SECRET`), com renovação automática de sessão.

Não há credencial para solicitar: você entra com o e-mail e a senha que já usa no toolbox, e o agente herda as **suas** permissões. Detalhes em [Instalação](/mcp/instalacao).

## Segurança

- **O agente opera com as suas permissões, não com as da organização.** Se você não enxerga um relatório no toolbox, o agente também não enxerga. Não existe escalonamento de privilégio por usar o MCP.
...
- Toda chamada é **auditada no backend** identificando o **usuário** que a originou, não uma credencial genérica de integração.
- No modo stdio, as credenciais ficam no **cliente** (Claude Desktop, Cursor, etc.), nunca no servidor MCP.
